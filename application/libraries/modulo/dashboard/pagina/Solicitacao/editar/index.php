...
<div class="py-5"> 
	<p class="lead">Você estar na pagina de  <strong><?= $c_class?></strong>.</p>
	<p class="lead" style="font-size:15px;">Para retornar ate o menu principal, click nesse link ao lado. <a href="<?= base_url('dashboard/'. strtolower ( $c_class)) ?>">Retornar para principal</a></p>
</div>	

<script  src="https://code.jquery.com/jquery-3.2.1.js"  crossorigin="anonymous"></script>
<script src="<?= base_url('public/js/chosen/chosen.jquery.js')?>"></script>

<div class="main"> <?php //print_r($c_campos) ?>
		<?= form_open_multipart($c_action) ?>
			<?php foreach ($c_campos as $index => $resultCampos):  ?> 
				<?php  $_dados = strval($resultCampos->name); //CONVERTENDO ARRAY PRA STRING ?> 
				<?php foreach ($w_dados as  $resultData):  //CONTEUDO DO BANCO ?>
				...
				<?php elseif($resultCampos->default == 1 && $resultCampos->name != "timestamp"): ?>
					...
					<?= form_dropdown($resultCampos->name, $$campoDrop  , $resultData->$campoDrop, array('class'=> ' form-control chosen' , 'tabindex'=>"15"));  ?>
